    ...
    if len(generated_mols) == 0:
        ...
        if len(novel_smiles) >= 5:
            ...
            for i in range(len(novel_smiles)):
                print(novel_smiles[i])
        print('\n')  
        # Compute the property scores of novel smiles (DISABLED due to RDKit incompatibility)
        if len(novel_smiles):
            print('[{}]: Property evaluation skipped (RDKit compatibility issue)'.format(args.properties))

            # Write the property scores into file
            if epoch is not None and time is not None:
                with open(PROPERTY_FILE, 'a+') as wf:
                    wf.write('{},{},{},{},{},{:.5f},{:.5f},{:.5f},{:.5f},{:.1f}\n'
.format(epoch+1, 'NA', 'NA', 'NA', 'NA',
...
def main():
    ...
    if args.gen_pretrain:
        ...
        gen.load_state_dict(torch.load(G_PRETRAINED_MODEL, map_location=torch.device('cpu')))
    ...
    if args.dis_pretrain:
        ...
        dis.load_state_dict(torch.load(D_PRETRAINED_MODEL, map_location=torch.device('cpu')))
    ...
    if args.adversarial_train:
        ...
        if args.dis_lambda:
            if not os.path.exists(TenGAN_D_MODEL):
                ...
                dis.load_state_dict(torch.load(TenGAN_D_MODEL, map_location=torch.device('cpu')))
                dis.to(DEVICE)
